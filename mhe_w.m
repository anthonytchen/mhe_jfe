...
%   bSnP -- binary; 1: both states and parameters; 0: only states to be updated
%
% Outputs:
...
if bSnP % estimate both state and parameters   
    ...
    opt = optimset('Display', 'iter', 'Algorithm', 'sqp', 'MaxFunEvals', 5e3, ...
        'MaxIter', 5e2, 'TolX', 1e-10, 'TolFun', 1e-4);
    %[x_w_p1, f_obj] = fmincon(@(x_w_p) mhe_obj(x_w_p, nx, x0, x0Q, p0, Cov_p, useq, yseq, fn_state, Cov_x, ...
    %    fn_obsv, Cov_y, t_sample), x_w_p0, [], [], [], [], [], [], [], opt);
    [x_w_p1, f_obj] = fminunc(@(x_w_p) mhe_obj(x_w_p, nx, x0, x0Q, p0, Cov_p, useq, yseq, fn_state, Cov_x, ...
        fn_obsv, Cov_y, t_sample), x_w_p0);
    ...
    opt = optimset('Display', 'iter', 'Algorithm', 'interior-point', 'MaxFunEvals', 5e4, ...
        'MaxIter', 5e4, 'TolX', 1e-8);
    %[x_w1, f_obj] = fmincon(@(x_w) mhe_obj_states(x_w, p, nx, x0, x0Q, useq, yseq, ...
    %    fn_state, Cov_x, fn_obsv, Cov_y, t_sample), x_w0, [], [], [],  [], [], [], [], opt);
    [x_w1, f_obj] = fminunc(@(x_w) mhe_obj_states(x_w, nx, x0, x0Q, p, useq, yseq, fn_state, Cov_x, ...
        fn_obsv, Cov_y, t_sample), x_w0);
